...
def energy_consumption_cell(csv_path, number_series_cells, number_parallel_cells, R0_coefficient, OCV_coefficient, SoC, mass, wind, angle):
    ...
    while SOC_now[-1]>5:
        ...
        new_SOC_2= SOC_now[-1] - (ocv_cell-math.sqrt(ocv_cell**2 - (4*power_demand*R0_cell)/(number_parallel_cells*number_series_cells)))/(2*R0_cell*36*capacity_cell)
...
# energy_consumption_cell('Cell_data/CELL_E_TEST_00.csv', 219,1,1,1, 30)
